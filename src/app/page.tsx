      ...
      <main className="flex-1 flex flex-col">
        ...
        <section className="relative overflow-hidden pt-12 pb-20 md:pt-20 md:pb-28 border-b border-slate-200 dark:border-slate-800/80">
          ...
          <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 relative z-10">
            ...
            <div className="mt-14 w-full max-w-6xl mx-auto rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0b0f1a] shadow-xl dark:shadow-2xl overflow-hidden">
              {/* Window Bar */}
              ...
              {/* Mock Code + Mock AI Dialog Split */}
              <div className="grid grid-cols-1 md:grid-cols-12 divide-y md:divide-y-0 md:divide-x divide-slate-200 dark:divide-slate-800">
                <div className="md:col-span-7 p-4 bg-slate-50 dark:bg-[#080c16] font-mono text-xs text-slate-800 dark:text-slate-300 leading-relaxed overflow-x-auto">
                  <div className="text-slate-400">// Level 3 Hint Pattern: HashMap two-pass complement lookup</div>
                  <div><span className="text-purple-600 dark:text-purple-400">import</span> java.util.*;</div>
                  <br />
                  <div><span className="text-purple-600 dark:text-purple-400">class</span> <span className="text-amber-600 dark:text-amber-400 font-bold">Solution</span> &#123;</div>
                  <div className="pl-4">
                    <span className="text-purple-600 dark:text-purple-400">public int</span>[] twoSum(<span className="text-purple-600 dark:text-purple-400">int</span>[] nums, <span className="text-purple-600 dark:text-purple-400">int</span> target) &#123;
                  </div>
                  <div className="pl-8 text-indigo-600 dark:text-indigo-400">
                    Map&lt;Integer, Integer&gt; map = <span className="text-purple-600 dark:text-purple-400">new</span> HashMap&lt;&gt;();
                  </div>
                  <div className="pl-8">
                    <span className="text-purple-600 dark:text-purple-400">for</span> (<span className="text-purple-600 dark:text-purple-400">int</span> i = 0; i &lt; nums.length; i++) &#123;
                  </div>
                  <div className="pl-12">
                    <span className="text-purple-600 dark:text-purple-400">int</span> complement = target - nums[i];
                  </div>
                  <div className="pl-12">
                    <span className="text-purple-600 dark:text-purple-400">if</span> (map.containsKey(complement)) &#123;
                  </div>
                  <div className="pl-16 text-emerald-600 dark:text-emerald-400">
                    <span className="text-purple-600 dark:text-purple-400">return new int</span>[] &#123; map.get(complement), i &#125;;
                  </div>
                  <div className="pl-12">&#125;</div>
                  <div className="pl-12">map.put(nums[i], i);</div>
                  <div className="pl-8">&#125;</div>
                  <div className="pl-8 text-slate-400"><span className="text-purple-600 dark:text-purple-400">return new int</span>[0];</div>
                ...
                <div className="md:col-span-5 p-4 bg-white dark:bg-[#0b0f1d] flex flex-col justify-between space-y-4">
                  <div className="space-y-3">
                    <div className="flex items-center space-x-2 text-xs font-semibold text-purple-600 dark:text-purple-400 font-mono">
                      <Sparkles className="w-3.5 h-3.5" />
                      <span>Socratic Guidance • Level 3</span>
                    </div>
                    <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
                      "Notice how checking <code className="text-indigo-600 dark:text-indigo-300">target - currentVal</code> in a <code className="text-amber-600 dark:text-amber-300">HashMap</code> reduces your time complexity from <strong>O(n²)</strong> down to <strong>O(n)</strong>."
                    </div>
                    <div className="flex items-center space-x-2 text-[11px] text-slate-500 font-mono">
                      <span>Speed: 2 ms (Beats 98.4%)</span>
                      <span>•</span>
                      <span>Memory: 43.8 MB</span>
                    </div>
                  </div>

                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <span className="text-xs text-slate-500 font-mono">Try this problem live</span>
                    <Link
                      href="/problems/two-sum-target"
                      className="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold flex items-center space-x-1 transition-all"
                    >
