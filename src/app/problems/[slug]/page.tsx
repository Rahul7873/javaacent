...
export default function ProblemWorkspacePage({
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-100 dark:bg-[#070a12] text-slate-900 dark:text-slate-100 overflow-hidden">
      <Navbar />

      {/* Top Workspace Action Ribbon */}
      <div className="h-12 bg-white dark:bg-[#0b0f1a] border-b border-slate-200 dark:border-slate-800/80 px-4 flex items-center justify-between shrink-0 shadow-sm">
        <div className="flex items-center space-x-3">
          <button
            onClick={handleBack}
            className="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-600 hover:text-slate-950 dark:text-slate-400 dark:hover:text-white border border-slate-200 dark:border-slate-800 transition-colors"
          ...
          <div className="flex items-center space-x-2">
            <h1 className="text-sm font-semibold text-slate-900 dark:text-white tracking-tight flex items-center space-x-2">
              <span>{problem.title}</span>
              {isSolvedAlready && (
                <span title="Solved">
                  <CheckCircle2 className="w-4 h-4 text-emerald-500 dark:text-emerald-400 inline shrink-0" />
                </span>
              )}
            </h1>
        ...
        <div className="flex items-center space-x-3">
          {/* Language Selector */}
          <div className="flex items-center space-x-1.5 bg-slate-100 dark:bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-800 text-xs text-slate-800 dark:text-slate-200">
            ...
            >
              {LANGUAGES.map(lang => (
                <option key={lang.id} value={lang.id} className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
                  {lang.label}
                </option>
              ))}
            </select>
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 transition-colors"
          ...
          <button
            ...
            className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all shadow-md ${
              isTutorOpen
                ...
                : 'bg-white hover:bg-slate-100 dark:bg-slate-800/90 dark:hover:bg-slate-700/80 text-purple-700 dark:text-purple-300 border border-purple-300 dark:border-purple-500/30'
            }`}
          >
            <Sparkles className="w-3.5 h-3.5 text-amber-500 dark:text-amber-300 animate-spin" />
            <span>AI Tutor</span>
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-3.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-100 border border-slate-300 dark:border-slate-700 text-xs font-medium disabled:opacity-50 transition-colors"
          >
            <Play className={`w-3.5 h-3.5 ${isRunning ? 'animate-spin' : 'text-slate-600 dark:text-slate-300'}`} />
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/2 flex flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090d17] overflow-hidden">
          {/* Left Tabs */}
          <div className="flex items-center px-4 bg-slate-50 dark:bg-slate-900/70 border-b border-slate-200 dark:border-slate-800 text-xs shrink-0">
            <button
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 text-sm text-slate-700 dark:text-slate-300">
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {problem.topics.map(t => (
                    <span
                      key={t}
                      className="px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 text-slate-700 dark:text-slate-300 text-xs font-mono"
                    >
                      {t}
                    </span>
                  ))}
                  <span className="px-2 py-0.5 text-xs text-slate-500 dark:text-slate-400 font-mono">
                    Acceptance: {problem.acceptanceRate}
                  </span>
                </div>

                {/* Description Body */}
                <div className="prose prose-slate dark:prose-invert prose-sm max-w-none leading-relaxed whitespace-pre-wrap text-slate-800 dark:text-slate-200">
                  {problem.description}
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {problem.examples.map((ex, i) => (
                    <div
                      key={i}
                      className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/80 space-y-2 font-mono text-xs"
                    >
                      <div className="text-indigo-600 dark:text-indigo-300 font-sans font-semibold text-xs">
                        Example {i + 1}:
                      </div>
                      <div>
                        <span className="text-slate-500">Input: </span>
                        <span className="text-slate-900 dark:text-slate-200 font-semibold">{ex.input}</span>
                      </div>
                      <div>
                      ...
                      {ex.explanation && (
                        <div>
                          ...
                          <span className="text-slate-700 dark:text-slate-300">{ex.explanation}</span>
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className="space-y-2">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-xs font-mono text-slate-700 dark:text-slate-300 pl-1">
            ...
            {activeLeftTab === 'hints' && (
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <details className="group border border-slate-200 dark:border-slate-800 rounded-lg p-3 bg-slate-50 dark:bg-slate-900/60">
                    <summary className="font-semibold text-xs text-slate-800 dark:text-slate-200 cursor-pointer flex items-center justify-between">
                      <span>Level 1: Guiding Question</span>
                      <span className="text-xs text-indigo-600 dark:text-indigo-400 group-open:rotate-90 transition-transform">▸</span>
                    </summary>
                    <p className="mt-2 text-xs text-slate-700 dark:text-slate-300 pl-2 border-l border-indigo-500/40">
                  ...
                  <details className="group border border-slate-200 dark:border-slate-800 rounded-lg p-3 bg-slate-50 dark:bg-slate-900/60">
                    ...
                    <pre className="mt-2 p-2 rounded bg-slate-900 text-slate-100 font-mono text-[11px] whitespace-pre-wrap overflow-x-auto">
            ...
            {activeLeftTab === 'solution' && (
              <div className="space-y-4">
                <div className="flex items-center space-x-4 p-3 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 font-mono text-xs">
                  <div>
                    <span className="text-slate-500">Time Complexity: </span>
                    <span className="text-indigo-600 dark:text-indigo-400 font-bold">{problem.timeComplexity}</span>
                  </div>
                  <div>
                    <span className="text-slate-500">Space Complexity: </span>
                    <span className="text-purple-600 dark:text-purple-400 font-bold">{problem.spaceComplexity}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <h4 className="text-xs font-semibold text-slate-800 dark:text-slate-300">Detailed Implementation:</h4>
                  <pre className="p-3 rounded-lg bg-slate-900 border border-slate-800 text-slate-100 font-mono text-xs whitespace-pre-wrap overflow-x-auto">
                    {problem.hints.level6}
                  </pre>
                </div>
              </div>
        ...
        <div className={`flex flex-col ${isTutorOpen ? 'w-2/6' : 'w-1/2'} transition-all overflow-hidden`}>
          ...
          <div className="h-[60%] p-2 bg-slate-100 dark:bg-[#080c16]">
            <MonacoEditor
              language={language}
              value={code}
          ...
          <div className="h-[40%] p-2 bg-slate-100 dark:bg-[#080c16]">
            <OutputDock
      ...
      {isJavaSheetOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#0b0f1a] border border-slate-200 dark:border-amber-500/40 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
            <div className="p-4 bg-slate-50 dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <h3 className="font-bold text-sm text-slate-900 dark:text-white">Java 17 Collections & DSA Cheat Sheet</h3>
                  <p className="text-[11px] text-amber-600 dark:text-amber-300/80 font-mono">Standard library references for competitive programming</p>
                </div>
              </div>
              <button
                onClick={() => setIsJavaSheetOpen(false)}
                className="p-1 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800"
              >
                ✕
              </button>
            </div>

            <div className="flex-1 overflow-y-auto p-5 space-y-4 font-mono text-xs text-slate-800 dark:text-slate-300">
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-amber-600 dark:text-amber-400 font-bold block">1. HashMap / HashSet</span>
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-amber-600 dark:text-amber-400 font-bold block">2. Deque (Stack & Queue)</span>
                <div className="text-slate-600 dark:text-slate-400 text-[11px]">
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-amber-600 dark:text-amber-400 font-bold block">3. PriorityQueue (Min / Max Heap)</span>
                <div className="text-slate-600 dark:text-slate-400 text-[11px]">
              ...
              <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1">
                <span className="text-amber-600 dark:text-amber-400 font-bold block">4. Strings & Sorting</span>
                <div className="text-slate-600 dark:text-slate-400 text-[11px]">
