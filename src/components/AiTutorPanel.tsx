...
export function AiTutorPanel({
  ...
  return (
    <div className="flex flex-col h-full bg-[#0c101d] border-l border-slate-800 shadow-2xl w-full">
      {/* Header */}
      <div className="p-3.5 border-b border-slate-800 bg-[#090d18] flex items-center justify-between">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <div className="flex items-center space-x-1.5">
              <h3 className="font-semibold text-sm text-slate-100 font-mono">Socratic AI Tutor</h3>
              <span className="px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded">
                Active
              </span>
            </div>
            <p className="text-[11px] text-slate-400">Progressive logic guidance</p>
          </div>
        </div>
      ...
      <div className="p-3 bg-slate-900/60 border-b border-slate-800/80">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[11px] font-medium text-slate-400">Teaching Hierarchy</span>
          <span className="text-[11px] font-mono text-indigo-400">Level {currentLevel} of 6</span>
        </div>
            ...
            return (
              <button
                ...
                className={`py-1.5 px-0.5 rounded text-center transition-all flex flex-col items-center group relative ${
                  isCurrent
                    ? 'bg-indigo-600 text-white font-bold ring-2 ring-indigo-400 ring-offset-1 ring-offset-slate-900'
                    : isCompleted
                    ? 'bg-indigo-950/80 text-indigo-300 border border-indigo-700/50 hover:bg-indigo-900/60'
                    : 'bg-slate-800/40 text-slate-500 border border-slate-800 hover:bg-slate-800 hover:text-slate-300'
                }`}
              >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, idx) => (
          ...
          >
            ...
            >
              {msg.hintLevel && (
                <div className="flex items-center space-x-1.5 mb-1.5 pb-1 border-b border-slate-700/50 text-[10px] font-mono text-indigo-300">
                  <span className="font-semibold">Level {msg.hintLevel}:</span>
        ...
        {isLoading && (
          <div className="flex items-start space-x-2.5">
            ...
            <div className="bg-slate-800/90 border border-slate-700/60 rounded-2xl rounded-tl-none px-3.5 py-2.5 text-xs text-slate-400 flex items-center space-x-2">
              <div className="w-2 h-2 rounded-full bg-indigo-400 animate-pulse" />
              <div className="w-2 h-2 rounded-full bg-indigo-400 animate-pulse delay-150" />
              <div className="w-2 h-2 rounded-full bg-indigo-400 animate-pulse delay-300" />
              <span className="font-mono text-[11px] text-slate-300">Synthesizing pedagogical hint...</span>
            </div>
          </div>
        )}
        <div ref={chatEndRef} />
      ...
      <div className="px-3 pt-2 pb-1 border-t border-slate-800/80 bg-slate-900/40">
        <div className="flex items-center space-x-1 mb-1.5 text-[10px] text-slate-400 font-mono">
          ...
          {suggestedPrompts.slice(0, 3).map((prompt, i) => {
            ...
            return (
              <button
                ...
                disabled={isLoading}
                className="text-[11px] px-2.5 py-1 rounded-full bg-slate-800/80 hover:bg-indigo-950/60 border border-slate-700/60 hover:border-indigo-500/40 text-slate-300 hover:text-indigo-300 transition-colors text-left"
              >
                {prompt}
              </button>
            );
      ...
      {/* Input Box */}
      <div className="p-3 bg-[#090d18] border-t border-slate-800">
        <form
          onSubmit={e => {
            e.preventDefault();
            handleSendMessage();
        ...
        >
          <input
            ...
            disabled={isLoading}
            className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"
          />
          <button
            type="submit"
            disabled={isLoading || !inputMessage.trim()}
            className="p-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 text-white disabled:text-slate-600 transition-colors"
          >
